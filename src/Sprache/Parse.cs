...
namespace Sprache.Binary
{
  ...
  public static partial class Parse
  {
    ...
    public static Parser<byte> Byte(Predicate<byte> predicate, string description)
    {
      ...
      return i =>
      {
        if (!i.AtEnd)
        {
          ...
          return Result.Failure<byte>(i, $"unexpected '{i.Current}'", new[] { description });
        }
    ...
    public static Parser<IEnumerable<byte>> ByteSequence(IEnumerable<byte> s)
    {
      ...
      return s.Select(Byte)
        .Aggregate(Return(Enumerable.Empty<byte>()), (a, p) => a.Concat(p.Once()))
    ...
    public static Parser<T> Named<T>(this Parser<T> parser, string name)
    {
      ...
      return i =>
        parser(i)
          .IfFailure(f =>
            f.Remainder.Equals(i) ? Result.Failure<T>(f.Remainder, f.Message, new[] { name }) : f
          );
    }
    ...
    public static Parser<T> Where<T>(this Parser<T> parser, Func<T, bool> predicate)
    {
      ...
      return i =>
        parser(i)
          .IfSuccess(s =>
            predicate(s.Value) ? s : Result.Failure<T>(i, $"Unexpected {s.Value}.", new string[0])
          );
    }
    ...
    )
    {
      ...
      return or(
        op.Then(opValue =>
          operand
            .Then(operandValue => ChainRightOperatorRest(operandValue, op, operand, apply, or))
