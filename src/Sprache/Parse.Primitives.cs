...
namespace Sprache.Binary
{
  partial class Parse
  {
    ...
    /// </summary>
    public static readonly Parser<byte> Int8 = from @byte in AnyByte select @byte;

    /// <summary>
    /// Parses a Short (2 bytes) from the input.
    /// </summary>
    public static readonly Parser<short> Int16 =
      from bytes in AnyByte.Repeat(2)
      select BitConverter.ToInt16(bytes.ToArray(), 0);

    /// <summary>
    /// Parses an UShort (2 bytes) from the input.
    /// </summary>
    public static readonly Parser<ushort> UInt16 =
      from bytes in AnyByte.Repeat(2)
    ...
    /// </summary>
    public static readonly Parser<int> Int32 =
      from bytes in AnyByte.Repeat(4)
      select BitConverter.ToInt32(bytes.ToArray(), 0);

    /// <summary>
    /// Parses an Int (4 bytes) from the input.
    /// </summary>
    public static readonly Parser<uint> UInt32 =
      from bytes in AnyByte.Repeat(4)
    ...
    /// </summary>
    public static readonly Parser<long> Int64 =
      from bytes in AnyByte.Repeat(8)
      select BitConverter.ToInt64(bytes.ToArray(), 0);

    /// <summary>
    /// Parses an ULong (8 bytes) from the input.
    /// </summary>
    public static readonly Parser<ulong> UInt64 =
      from bytes in AnyByte.Repeat(8)
    ...
    /// </summary>
    public static readonly Parser<float> Single =
      from bytes in AnyByte.Repeat(4)
    ...
    /// Parses an String from the input.
    /// </summary>
    public static Parser<string> FixedString(int length) =>
      from bytes in AnyByte.Repeat(length)
      select Encoding.UTF8.GetString(bytes.ToArray());
    ...
    /// </summary>
    public static readonly Parser<string> StringZeroTerminated =
      from bytes in AnyByte.Until(Byte(0))
      select Encoding.UTF8.GetString(bytes.ToArray());

    [Obsolete]
    public static readonly Parser<byte[]> ByteArray =
      from length in AnyByte
      from bytes in AnyByte.Repeat(length)
