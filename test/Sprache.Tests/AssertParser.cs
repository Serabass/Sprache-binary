...
namespace Sprache.Binary.Tests
{
  ...
  {
    public static void SucceedsWithOne<T>(
      Parser<IEnumerable<T>> parser,
    ...
    {
      parser
        .TryParse(new MemoryStream(input.ToArray()))
        .IfFailure(f =>
        {
    ...
    public static void FailsWith<T>(
      Parser<T> parser,
      Stream input,
      Action<IResult<T>> resultAssertion
    )
    {
      parser
        .TryParse(input)
