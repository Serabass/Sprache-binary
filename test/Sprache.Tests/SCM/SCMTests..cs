...
namespace Sprache.Binary.Tests.SCM
{
  public class SCMTests
  {
    ...
    public void ReadSCMTest()
    {
      ...
      var nameThread = from name in String8 select name;

      var opcode = from op in Parse.UInt16 select (SCMOpcode)op;

      var scmDoc =
        from _ in Parse.AnyByte.Repeat(0x9AE4)
        from op in opcode
      ...
      var p = from opcode1 in scmDoc select opcode1;
    }
  }
}
