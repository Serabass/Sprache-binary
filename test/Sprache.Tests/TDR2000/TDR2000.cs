...
public class DirParser : IDisposable
{
  ...
  static readonly Parser<char> TChar =
    from ch in Parse.AnyByte
    from unk in Parse.Bytes(0x40, 0xC0)
    select (char)ch;

  public static readonly Parser<char> TLastChar =
    from ch in Parse.AnyByte
    from end in Parse.Byte(0x08)
    select (char)ch;

  public static readonly Parser<string> TString =
    from chars in TChar.Many()
    from end in TLastChar
    select new string(chars.ToArray().Concat(new[] { end }).ToArray());

  public static readonly Parser<TBlock> TBlock =
    from fileName in TString
  ...
  public static readonly Parser<TBlock[]> TDocument =
    from blocks in TBlock.Many()
